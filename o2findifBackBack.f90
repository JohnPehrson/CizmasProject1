FUNCTION o2findifBackBack(IMax,JMax,i,j,dz,dn,XORY,alpha,beta,gamma) result(XORYnew)
    IMPLICIT NONE
    !inputs
   ...
   !temporary array to hold values nicely
    REAL, DIMENSION(5) :: abb
    !output
    REAL :: XORYnew ! outputs the new x or y value


    abb = [-2.*alpha/(dz**2.)+2.*beta/(dz*dn),-2.*gamma/(dn**2.)+2.*beta/(dn*dz),-2.*beta/(dz*dn),&
