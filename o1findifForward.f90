FUNCTION o1findifForward(IMax,JMax,i,j,dz,dn,XORY,switchnum) result(findif)
   IMPLICIT NONE
    !inputs
    ...
    IF (switchnum == 1) THEN
        findif = (XORY(i+1,j)-XORY(i,j))/(dz)
     ELSE IF (switchnum == 2) THEN
        findif = (XORY(i,j+1)-XORY(i,j))/(dn)
     ELSE
        ! print error
        WRITE(*,*) "Bad switchnumber in 1st order forward difference"
     END IF


end FUNCTION o1findifForward